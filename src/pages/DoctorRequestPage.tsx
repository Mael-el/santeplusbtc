...
export default function DoctorRequestPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.speciality.trim()) {
      setError('La spécialité ou fonction est obligatoire.');
      return;
    }
    ...
    try {
      await fetch('/api/contact/professional', {
        ...
        body: JSON.stringify({
          ...
          specialty: form.speciality.trim(),
        }),
      }).catch(() => undefined);
      setSuccess(true);
      setForm({ fullName: '', email: '', phone: '', speciality: '' });
    } catch {
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#ECFDF5] via-white to-[#E3F6EC] px-4 py-10">
      <div className="mx-auto max-w-xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-emerald-700">Demande d’accompagnement médecin</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="rounded-[28px] border border-emerald-200 bg-white p-6 shadow-[0_20px_50px_rgba(0,168,107,0.08)] sm:p-8">
          <h2 className="mb-6 text-2xl font-black text-emerald-900">Médecin</h2>
          ...
          <div className="space-y-4">
            ...
            <label className="block text-sm font-bold text-slate-700">
              ...
              <input
                ...
                className="mt-2 w-full rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3 outline-none transition focus:border-emerald-500"
                placeholder="Médecin généraliste / Cardiologie"
              />
            </label>
          </div>
        ...
        <div className="mt-8 rounded-[28px] border border-emerald-200 bg-white p-6 shadow-[0_20px_50px_rgba(0,168,107,0.08)]">
          ...
          <p className="mt-2 text-sm text-slate-600">Pour tester l’espace médecin sans attendre le contact commercial.</p>

          <div className="mt-4 rounded-2xl bg-emerald-50 border border-emerald-200 p-4">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-emerald-700">Espace médecin</p>
            <div className="mt-3 space-y-2 text-sm text-slate-700">
              <p><span className="font-bold">Email :</span> medecin@demo.santeplus.bj</p>
              <p><span className="font-bold">Mot de passe :</span> Demo123!</p>
            </div>
            <ul className="mt-3 list-disc pl-5 text-sm text-slate-600">
              <li>Consulter les dossiers patients</li>
              <li>Générer des ordonnances</li>
              <li>Suivre les rendez-vous</li>
            </ul>
          </div>
        </div>
      </div>
